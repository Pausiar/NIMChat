...
export function AgentsModePlaceholder({ onBack }: { onBack?: () => void }) {
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-6 bg-[#0a0a0b] pb-24 text-center">
      ...
      <div className="max-w-md space-y-2">
        <h1 className="text-2xl font-semibold text-white">Modo Agents</h1>
        <p className="text-sm text-white/60">
          Orquesta varios agentes especializados (research, code, review…) que
          colaboran entre sí para resolver tareas complejas. Próximamente.
        </p>
      </div>
      <div className="flex flex-wrap items-center justify-center gap-2 text-xs text-white/40">
        <Feature icon={<Sparkles className="h-3 w-3" />} label="Planner + workers" />
        <Feature icon={<Sparkles className="h-3 w-3" />} label="Memoria compartida" />
        <Feature icon={<Sparkles className="h-3 w-3" />} label="Tools por rol" />
      </div>
      {onBack && (
